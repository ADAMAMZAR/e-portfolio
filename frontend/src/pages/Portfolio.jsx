...
export function Portfolio() {
  ...
  const filteredProjects = useMemo(() => {
    ...
    return allProjects.filter(p => p.category === activeTag);
  }, [state.projects, state.status, activeTag]);

  const tags = useMemo(() => {
    const allProjects = state.status === "ready" ? state.projects : [];
    return ["All", ...new Set(allProjects.map(p => p.category).filter(Boolean))];
  }, [state.projects, state.status]);
